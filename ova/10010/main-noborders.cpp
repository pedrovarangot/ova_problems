    ...
    switch(dir) {
        ...
        case LD2UR:
            ...
            for(j = 1; j <= cols - len; j++) {
                for(i = rows - 1; i >= rows - len; i--) {
                    //cout << "LD2UR: " << i << " " << j + (rows - 1 - i) << endl; 
                    //cout << "POW: " << len - (rows - 1 - i) << endl;
...
int main()
{
    ...
    while(cases--) {
        ...
        memset(h, 0, 8*50*50*50*sizeof(unsigned int));
        //print_h(h, rows, cols);
...
#define FOUNDWORD { do { if(row > i or (row == i and j < col) ) { row = i; col = j; } } while(0); };
            for(int i = 0; i < rows; i++) {
                for(int j = 0; j < cols; j++) {
                    if(h[LEFT2RIGT][it->size()][i][j] == word_hash) { FOUNDWORD; }
                    if(h[UP2DOWN][it->size()][i][j] == word_hash) { FOUNDWORD; }
                    if(h[LU2DR][it->size()][i][j] == word_hash) { FOUNDWORD; } 
                    if(h[LD2UR][it->size()][i][j] == word_hash) { FOUNDWORD; } 
                    o_i = i; o_j = j;
                    if(h[LEFT2RIGT][it->size()][i][j] == r_word_hash) { j += it->size() - 1; FOUNDWORD; }
                    if(h[UP2DOWN][it->size()][i][j] == r_word_hash) { i += it->size() - 1; FOUNDWORD; }
                    if(h[LU2DR][it->size()][i][j] == r_word_hash) { j += it->size() -1; i += it->size() - 1; FOUNDWORD; } 
                    if(h[LD2UR][it->size()][i][j] == r_word_hash) { j += it->size() -1; i -= it->size() - 1; FOUNDWORD; } 
                    i = o_i; j = o_j;
                }
            }
